...
const [dealsRes,discRes] = await Promise.all([
brandApi.list(),
brandApi.discover()
]);
...
...newDeal,
value: parseFloat(newDeal.value) || undefined
});
...
{deals.length} active deals · {formatINR(totalRevenue)} revenue
</p>
...
className="text-[12px] px-3 py-1.5 rounded-md capitalize"
style={{
...
className="border border-gray-200 rounded-lg px-3 py-2 text-[12px] outline-none w-full"
/>
...
onChange={e=>setNewDeal(p=>({...p,deal_type:e.target.value}))}
className="border border-gray-200 rounded-lg px-3 py-2 text-[12px] w-full"
>
...
<button
onClick={addDeal}
className="flex-1 py-2 rounded-lg text-white text-[12px]"
style={{background:'#534AB7'}}
>
Add
</button>
...
className="flex-1 py-2 rounded-lg text-[12px] border border-gray-200 text-gray-500"
>
Cancel
</button>
...
className="text-[10px] px-2 py-0.5 rounded-md text-gray-400 border border-gray-100 hover:text-red-400"
>
Remove
</button>
...
<div className="w-9 h-9 rounded-lg bg-purple-100 text-purple-700 text-[11px] font-medium flex items-center justify-center">
